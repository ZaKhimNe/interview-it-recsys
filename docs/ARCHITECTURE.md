# Kiến trúc dự án InternHub

> Tài liệu mô tả cấu trúc mã nguồn, mục đích từng file, luồng dữ liệu và phân công nhóm.

---

## 1. Tổng quan

**InternHub** là hệ thống hỗ trợ ôn luyện phỏng vấn kỹ thuật cho các vị trí Data (DA / DE / DS) sử dụng AI. Hệ thống cho phép:

- Đánh giá năng lực ban đầu qua bộ câu hỏi phân cấp độ khó
- Mô phỏng buổi phỏng vấn kỹ thuật với AI chấm điểm tự động
- Tính toán vector năng lực 6 chiều và cập nhật theo thời gian
- Gợi ý câu hỏi và lộ trình học tập phù hợp với điểm yếu của ứng viên
...
|------|----------|----------|
| **Giao diện** | `streamlit 1.45` | Framework UI web dạng Python |
| **Biểu đồ** | `plotly 6.1` | Radar chart, bar chart năng lực |
| **AI / LLM** | `google-generativeai 0.8` | Gọi API Google Gemini |
| **LLM Orchestration** | `langchain 0.3` | Chuỗi prompt, pipeline RAG |
| **Vector DB** | `chromadb 1.0` | Lưu embedding câu hỏi cho RAG |
| **Validation** | `pydantic 2.11` + `jsonschema 4.24` | Kiểm tra schema dữ liệu |
| **Data** | `pandas 2.0+`, `numpy 1.24+` | Xử lý bảng dữ liệu và vector số |
| **ML utils** | `scikit-learn 1.6` | Tính độ tương đồng, chuẩn hóa |
| **Logging** | `loguru 0.7` | Ghi log có cấu trúc với rotation |
| **Test** | `pytest 8.4` | Unit test và integration test |
| **Config** | `python-dotenv 1.1` | Đọc biến môi trường từ `.env` |
...
1. **App Lead không viết logic trong UI** — yêu cầu hàm từ Data/AI Lead, gọi qua interface rõ ràng
2. **Luôn validate dữ liệu** — dùng `schema_validator` trước khi lưu vào `data/` hoặc gửi cho AI
3. **Prompt tập trung** — mọi thay đổi prompt thực hiện trong `ai_core/prompts.py`, không hardcode trong UI
4. **Không hardcode hằng số** — dùng `config.py` và `.env` cho API key, path, tham số
5. **Chạy test** — đảm bảo `pytest tests/ -v` pass trước khi merge
...
|--------|------|-----------|
| UI skeleton | `ui/onboarding.py`, `ui/sidebar.py`, `ui/screens.py` | ✅ Hoàn thành |
| Session management | `ui/session_manager.py` | ✅ Hoàn thành |
| Competency engine | `core/competency_engine.py` | ✅ Hoàn thành |
| Test generator | `core/test_generator.py` | ✅ Hoàn thành |
| Data loader | `core/data_loader.py` | ✅ Hoàn thành |
| Schema validator | `core/schema_validator.py` | ✅ Hoàn thành |
| Prompt templates | `ai_core/prompts.py` | ✅ Hoàn thành |
| Question bank | `data/mock/mockdata.json` | ✅ 1700+ câu hỏi |
| AI grader | `ai_core/grader.py` | ❌ TODO (AI Lead) |
| AI recommender | `ai_core/recommender.py` | ❌ TODO (AI Lead) |
| Assessment page | `ui/pages/assessment.py` | ❌ Cần hoàn thiện |
| Interview page | `ui/pages/interview.py` | ❌ Cần hoàn thiện |
| Dashboard page | `ui/pages/dashboard.py` | ❌ Cần hoàn thiện |
| RAG / ChromaDB | `data/vector_store/` | ❌ Chưa tích hợp |
| Unit tests | `tests/` | ❌ Cần bổ sung |
