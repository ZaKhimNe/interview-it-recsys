...
def show_onboarding():
    ...
    Sau khi chọn xong, role sẽ được lưu vào st.session_state.user_role.
    """

    st.title("🎯 AI Tech Interview Prep")

    st.markdown(
        """
        Hệ thống đánh giá năng lực và gợi ý lộ trình học tập cá nhân hóa.

        Ứng dụng sẽ giúp bạn:

        - Đánh giá năng lực hiện tại
        - So sánh với yêu cầu của role
        - Gợi ý kỹ năng cần cải thiện
        - Luyện tập câu hỏi theory và coding
        """
    )
    ...
    if st.button("🚀 Bắt đầu ngay"):
        if selected_role == "":
            st.warning("Vui lòng chọn role trước khi bắt đầu.")
        else:
